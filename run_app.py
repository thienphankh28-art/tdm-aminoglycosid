            ...
            if parse_version(remote_version_str) > parse_version(current_version):
                ...
                for file_name in FILES_TO_UPDATE:
                    ...
                    if file_res.status_code == 200:
                        target_path = os.path.join(BASE_DIR, file_name)
                        # Đoạn mới: Ép buộc đồng bộ chuẩn xuống dòng Windows (CRLF) tránh lỗi Mixed Newlines
                        with open(target_path, "w", encoding="utf-8") as fw:
                            # Tách mọi dòng bất kể là \n hay \r\n, sau đó ép buộc toàn bộ file dùng chuẩn \r\n của Windows
...
if __name__ == "__main__":
    ...
    # Luôn load file app.py nằm ở thư mục thực tế bên ngoài (nơi có thể ghi đè cập nhật)
    ...
    sys.argv = [
        ...
        "--server.headless=true",  # Tắt tính năng tự mở trình duyệt của Streamlit
        ]
