...
with tab1:
    ...
    st.divider()
    st.subheader("📄 Xuất báo cáo hội chẩn TDM (PDF) theo CSDL bệnh nhân")

    if st.button("📥 Tạo và Tải file báo cáo PDF từ CSDL"):
        if not msyt_input:
            st.error("Vui lòng nhập MSYT để xuất báo cáo.")
        else:
            try:
                ...
                # Hàm loại bỏ dấu tiếng Việt để tương thích tuyệt đối với font chuẩn của FPDF
                def clean_text(text):
                    if not text:
                        return ""
                ...
                patient_info = patient_res.data[0] if patient_res.data and len(patient_res.data) > 0 else {}

                # Lấy thông số từ bảng patients, có dự phòng (fallback) lấy từ giao diện nếu bảng trống
                p_age = patient_info.get("age") or (age if 'age' in locals() else 'N/A')
                p_gender = patient_info.get("gender") or (gender if 'gender' in locals() else 'N/A')
                p_weight = patient_info.get("weight_kg") or (weight_kg if 'weight_kg' in locals() else 0)
                p_height = patient_info.get("height_cm") or (height_cm if 'height_cm' in locals() else 0)
                p_bmi = patient_info.get("bmi") or (bmi if 'bmi' in locals() else 0)
                p_ibw = patient_info.get("ibw") or (ibw if 'ibw' in locals() else 0)
                p_dosing_weight = patient_info.get("dosing_weight") or (dosing_weight if 'dosing_weight' in locals() else 0)

                # 2. Truy vấn toàn bộ lịch sử TDM từ bảng 'tdm_history' trên Supabase
                history_res = db.supabase.table("tdm_history").select("*").eq("msyt", msyt_input).order("tdm_date", desc=True).execute()
                ...
                pdf.cell(0, 10, clean_text("BAO CAO LICH SU THEO DOI NONG DO THUOC (TDM) AMINOGLYCOSID"), ln=True, align="C")
                pdf.ln(5)

                # 1. Thông tin hành chính bệnh nhân (Lấy chuẩn từ table patients)
                pdf.set_font("Arial", "B", 11)
                pdf.cell(0, 8, clean_text("1. Thong tin hanh chinh benh nhan"), ln=True)
                pdf.set_font("Arial", "", 10)
                pdf.cell(0, 6, clean_text(f"- Ma so y te (MSYT): {msyt_input}"), ln=True)
                pdf.cell(0, 6, clean_text(f"- Tuoi: {p_age}   |   Gioi tinh: {p_gender}   |   Can nang: {p_weight} kg   |   Chieu cao: {p_height} cm"), ln=True)
                pdf.cell(0, 6, clean_text(f"- Chi so nhan: BMI = {p_bmi:.2f} kg/m2, IBW = {p_ibw:.2f} kg, Can nang tinh lieu = {p_dosing_weight:.2f} kg"), ln=True)
                pdf.ln(5)

                # 2. Danh sách các block dữ liệu TDM theo thời gian (Lấy từ table tdm_history)
                pdf.set_font("Arial", "B", 11)
                if records:
                    pdf.cell(0, 8, clean_text(f"2. Chi tiet cac dot TDM da thuc hien ({len(records)} ban ghi tu CSDL)"), ln=True)
                    ...
                    for idx, row in enumerate(records, 1):
                        ...
                        pdf.cell(0, 6, clean_text(f"--- Dot {idx} | Ngay TDM: {row.get('tdm_date', 'Chong cap nhat')} ---"), ln=True)
                        
                        pdf.set_font("Arial", "", 10)
                        pdf.cell(0, 5, clean_text(f"  + Duoc dong hoc (PK): Ke = {ke_val:.4f} h-1, t1/2 = {thalf_val:.2f} gio, Vd = {vd_val:.2f} lit"), ln=True)
                        pdf.cell(0, 5, clean_text(f"  + Nong do thuc te: Peak = {peak_val:.2f} ug/mL, Trough = {trough_val:.3f} ug/mL"), ln=True)
                        pdf.cell(0, 5, clean_text(f"  + Khuyen nghi lieu moi: {dose_val} mg (tau = {tau_val}h, t' = {t_inf_val}h)"), ln=True)
                        pdf.cell(0, 5, clean_text(f"  + Du doan tai CSS: Cp_pred = {cp_pred_val:.2f} ug/mL, Ctr_pred = {ctr_pred_val:.3f} ug/mL"), ln=True)
                        pdf.ln(3)
                else:
                    pdf.set_font("Arial", "", 10)
                    pdf.cell(0, 6, clean_text("Chua co lich su TDM nao trong CSDL cho MSYT nay."), ln=True)

                pdf.ln(5)
                # Phần chữ ký và ngày tháng lập báo cáo
                pdf.set_font("Arial", "I", 10)
                pdf.cell(100, 6, "", ln=0)
                pdf.cell(90, 6, clean_text(f"Ngay lap bao cao: {datetime.date.today().strftime('%d/%m/%Y')}"), ln=True, align="C")
                pdf.cell(100, 6, "", ln=0)
                pdf.cell(90, 6, clean_text("Bac si / Duoc si lam sang"), ln=True, align="C")

                # Tạo file PDF tạm để tải xuống
                ...
                st.download_button(
                    label="📥 Tải xuống file báo cáo PDF tổng hợp từ CSDL",
                    data=pdf_bytes,
                    file_name=f"Bao_cao_CSDL_{msyt_input}.pdf",
                    mime="application/pdf"
                )
                st.success("Đã tạo báo cáo PDF từ CSDL thành công!")
                os.unlink(tmp_path)

            except Exception as e:
                st.error(f"Lỗi khi tạo file PDF từ CSDL: {e}")
...
with tab2:
    ...
    st.subheader("📈 Biểu đồ xu hướng TDM theo bệnh nhân")

    if lookup_tab2:

        try:

            # Truy vấn dữ liệu từ bảng tdm_history trên Supabase theo MSYT

            response = db.supabase.table("tdm_history").select("*").eq("msyt", lookup_tab2).execute()

            df_patient_trend = pd.DataFrame(response.data) if response.data else pd.DataFrame()

        except Exception as e:

            st.error(f"Lỗi khi tải dữ liệu từ Supabase: {e}")

            df_patient_trend = pd.DataFrame()

        if not df_patient_trend.empty:

            # Sắp xếp theo ngày TDM

            df_patient_sorted = df_patient_trend.sort_values("tdm_date")

            col_t1, col_t2 = st.columns(2)

            with col_t1:

                st.markdown(f"**Biểu đồ nồng độ thuốc thực tế (MSYT: {lookup_tab2})**")

                fig_drug = go.Figure()

                if "true_peak" in df_patient_sorted.columns and "true_trough" in df_patient_sorted.columns:

                    fig_drug.add_trace(go.Scatter(

                        x=df_patient_sorted["tdm_date"], y=df_patient_sorted["true_peak"],

                        mode='lines+markers', name='C đỉnh thực tế (Peak)', line=dict(color='red', width=2)

                    ))

                    fig_drug.add_trace(go.Scatter(

                        x=df_patient_sorted["tdm_date"], y=df_patient_sorted["true_trough"],

                        mode='lines+markers', name='C đáy thực tế (Trough)', line=dict(color='blue', width=2)

                    ))

                    fig_drug.update_layout(

                        xaxis_title="Ngày TDM", yaxis_title="Nồng độ (µg/mL)",
                    ...
                    st.info("Chưa có dữ liệu nồng độ đỉnh/đáy thực tế.")

            with col_t2:

                st.markdown(f"**Biểu đồ liều mới khuyến nghị (MSYT: {lookup_tab2})**")

                fig_dose = go.Figure()

                if "new_dose" in df_patient_sorted.columns:

                    fig_dose.add_trace(go.Scatter(

                        x=df_patient_sorted["tdm_date"], y=df_patient_sorted["new_dose"],

                        mode='lines+markers', name='Liều mới (mg)', line=dict(color='green', width=2)

                    ))

                    fig_dose.update_layout(

                        xaxis_title="Ngày TDM", yaxis_title="Liều (mg)",
                    ...
                    st.info("Chưa có thông tin liều mới.")

        else:

            st.info("Không có bản ghi TDM nào để vẽ biểu đồ cho MSYT này.")

    else:

        st.info("Vui lòng nhập MSYT ở ô tra cứu phía trên để xem biểu đồ xu hướng.")
